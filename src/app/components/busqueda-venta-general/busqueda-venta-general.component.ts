...
import { CompraGeneralService } from 'src/app/services/compra-general.service';/* PaltaChilenaService */
import{ToastrService} from 'ngx-toastr';




import {CpcPrincipal} from '../../models/cpcPrincipal';/* CpcPrincipal */
import {VpcPrincipal} from '../../models/vpcPrincipal';/* VpcPrincipal */
import { Abonos_Vpc } from '../../models/abonosVpc';/* Abonos_Vpc */



@Component({
  selector: 'app-busqueda-venta-general',
  templateUrl: './busqueda-venta-general.component.html',
  styleUrls: ['./busqueda-venta-general.component.css']
})
export class BusquedaVentaGeneralComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.compraGeneralService.getAllSells()
    .subscribe(
      res => {
...
id_venta_general:any;
  ...
  Search(){
    ...
    if(this.id_venta_general == ""){
      this.ngOnInit();
    }
    else{
      this.getAllSells = this.getAllSells.filter(res => {
        return res.id_venta_general.toString().match(this.id_venta_general.toString());
      })
    }
  }
  ...
  menuOpciones(
    ...
    costos_adicionales:any,
      ...
      this.selectedIdVentaGeneral = id_venta_general;
  ...
  obtenerDatos(){
    this.compraGeneralService.getSellDetail(this.selectedIdVentaGeneral)
    .subscribe(
      res => {
        console.log(res);
  ...
  // obtenerDatosAbonos(){
  //   this.compraGeneralService.GetDetailAbonoVpc(this.selectedIdVpc)
  //   .subscribe(
  //     res => {
  //       console.log("abonos");
...
//   }
  ...
  obtenerDatosAdicionales(){
    this.compraGeneralService.getSellAditionalCost(this.selectedIdVentaGeneral)
    .subscribe(
      res => {
        console.log(res);
  ...
  correlativoFuncion()
  {
    ...
    for (var i =0; i< this.ventaPaltaChilenaCorrelativo.length ; i++) {
      ...
      if(this.ventaPaltaChilenaCorrelativo[i].nombre == this.selectedNombre){
        ...
        if(this.ventaPaltaChilenaCorrelativo[i].id_venta_general == this.selectedIdVentaGeneral){
          this.countProcesCorrelative=this.countProcesFinal+1;
          console.log("identificador", this.countProcesCorrelative);
        }
      }
    }
  }
...
// }
  ...
  reporteGeneralPDF(){
    ...
    doc.text('DETALLE DE VENTA - VENTA GENERAL', 70, 65);
